...
/**
 * 배열을 활용 클래스를 설계 
 * 물론 이미 자바 표준 API 개발자들이 잘 만들어 준 클래스 들이 존재 한다. 
 * 하지만 직접 기능을 확장해서
 * 만들어 보자.
 */
public class TencoIntArray {
	...
	// 기능 설계
	// 배열 요수 제일 뒤에 값을 추가하는 기능을 가진다.
	public void addElenent(int inputData) {
		// 방어적 코드 필요
		if (count >= ARRAY_SIZE) {
	...
	// 배열에 요수를 추가하는 기능
	public void insertElement(int postion, int intputData) {
		// 방어적 코드 1
		if (count >= ARRAY_SIZE) {
			System.out.println("메모리 공간이 가득 찼습니다.");
			return;
		...
		// 방어적 코드 2
		// 10 < 0
		if (postion < 0 || ARRAY_SIZE < postion) {
			System.out.println("지정한 인덱스 번호가 잘못 되었습니다.");
		}
		// 요청값 : postion -> 3
		// [11, 12, 13, []14, 15]
		// [11, 12, 13, [], 14, 15]
		for (int i = count - 1; i >= postion; i--) {
			intArr[i + 1] = intArr[i]; // 하나씩
			// intArr[5] = 15; 수행 1
			// intArr[4] = 14; 수행 2
		}
		intArr[postion] = intputData;
		count++;
	}
	// 지정한 인덱스 번호에 요소를 삭제 하기
	public void removeElement(int postion) {
		...
		// intArr[postion]; --> 사용자가 요청한 인덱스 번호는 0 입니다.
		// [100] [200] [300] [400]
		// [200] [300] [400] [] 땡겨줘야 한다
	...
	// 현재 요소가 하나도 없는 상태이다. boolean 일땐 isEmpty 라는걸 자주쓴다
